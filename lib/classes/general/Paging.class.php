...
class Paging
{
	 ...
	 *@ return start value
	 */
	public function setAjax($val = false) {
		$this->ajax = $val;
	}
	...
	public function displayPaging()
	{
		...
		#echo $start_loop;exit;
		if($start_loop > $page_limit)
		{
			$prev_loop = $start_loop - 1;
			if ($this->ajax == true) {
				$this->page_string.='<a href="javascript:void(0);" onclick="setPaging('.($this->start - 1).')" class="paginglink" title="Previous"><b>Previous</b></a>';
		...
		for($loop=1 ; $loop<=$loop_limit ; $loop++)
		{
			...
			{
				if($this->ajax == true) {
					//$this->page_string.='<a href="'.$pagelink.'"  class="paginglink" title="'.$start_loop.'"><b>'.$start_loop.'</b></a>';
					$this->page_string .= '<a href="javascript:void(0);" onclick="setPaging('.($start_loop).')"  class="paginglink" title="'.$start_loop.'"><b>'.$start_loop.'</b></a>';
				} else {
		...
		{
			if ($this->ajax == true) {
				$this->page_string.='<a href="javascript:void(0);" class="paginglink" title="Next" onclick="setPaging('.$start_loop.')"><b>Next</b></a>';
	...
	public function replaceUrl($url,$uKey,$uVal)
	{
		...
		else
		{
			...
			$r = ''; $set = 0;
			foreach($_REQUEST as $key => $val) {
				if($_REQUEST[$key] == $_POST[$key] && $key!='' && $_POST[$key]!='') {
					if($key == $uKey) {
						$val = $uVal;
						$set = 1;
					}
