...
class ImageMagick {
	...
	function ImageMagick($imagemagickinstalldir, $temp_gallery) {
		$this->temp_dir = $temp_gallery;
		$this->imagemagickdir = $imagemagickinstalldir;
	}

	/*
	 Places file in $this->temp_dir
	Gets the imagedata and stores it in $this->image_data

	*/

	function loadByFilePath($filepath) {
		$this->temp_file = basename($filepath);
		//echo $this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file;exit;
		if (!copy($filepath, $this->temp_dir . '/tmp' . $this->count . '_' . $this->temp_file)) {
			//echo $this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file;exit;
	...
	/*
	 Places uploaded file in $this->temp_dir
	Gets the imagedata and stores it in $this->image_data
	$filedata = $_FILES['file1']
	*/

	function loadByFileData($filedata) {
		$this->temp_file = basename($filedata['name']);
		//	echo $this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file;exit;
		if (!@copy($filedata['tmp_name'], $this->temp_dir . '/tmp' . $this->count . '_' . $this->temp_file))
			die("Imagemagick: Upload failed");
		$this->file_history[] = $this->temp_dir . '/tmp' . $this->count . '_' . $this->temp_file;
		$this->GetSize();
	}
