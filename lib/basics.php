 ...
 */
if (!function_exists("http_redirects")) {
	function http_redirects($url)
	{
		header("location:".$url);
		exit;
	}
...
function __autoload($class_name) {
	...
	/*$con = @strtolower($class_name{0});
	$cat = @substr($class_name,1);
	$class_name = $con.$cat;*/

	if(!is_included_files(LIB . $class_name . '.php') && file_exists(LIB . $class_name . '.php'))
		require_once (LIB . $class_name . '.php');
	else if(!is_included_files(GENERAL . $class_name . '.Class.php5') && file_exists(GENERAL . $class_name . '.Class.php5'))
		require_once(GENERAL. $class_name . '.Class.php5');
	else if(!is_included_files(APPLICATION . $class_name . '.Class.php5') && file_exists(APPLICATION . $class_name . '.Class.php5'))
		require_once(APPLICATION. $class_name . '.Class.php5');

}

function is_included_files($file)
{
...
function Add_Tield(&$item)
{
	if(!strstr($item,'`'))
	{
		$exeption_expression = preg_match_all("/\((.*)\)/", $item, $matches);
		if($matches[1][0]=='')
		{
			...
			{
				$ret_arr[$i] = '`'.$item_arr[$i].'`';
			}
		}
		//echo @implode(".",$ret_arr);exit;
		return $item = @implode(".",$ret_arr);
	}
}
function attachString($value, $key , $finalString)
{
	$finalString.= Add_Tield($key)." = '".trim($value)."', ";
}
function generateOption($value, $key, $columData)
{
	if(isset($value[$columData[2][2]][$columData[2][0]]))
	{
		if($value[$columData[2][2]][$columData[2][0]]==$columData[1])
			echo $strOpt.='<option value="'.$value[$columData[2][2]][$columData[2][0]].'" selected>'.$value[$columData[2][2]][$columData[2][1]].'</option>';
		else
			echo $strOpt.='<option value="'.$value[$columData[2][2]][$columData[2][0]].'" >'.$value[$columData[2][2]][$columData[2][1]].'</option>';

	}
}
function createRecordSet($value, $field, $database)
{
	$finfo = $database[1]->fetch_field_direct($database[3]);
	if($finfo->orgtable)
	...
	if($table=='')
		$table = $database[4];
	$database[0][$database[2]][$table][$field]= $value;
	$database[3]++;
	...
	/*$i=0;
	 ...
	 {
	$finfo = $results->fetch_field_direct($i);
	if($finfo->orgtable)
		$table = $finfo->orgtable;
	else
		$table = $finfo->table;
...
{
	if(count($_SESSION['QueryLog'])>0)
	{

		$str_query='<table align="left" cellspacing="1"border="1">';
		$str_query.='<tr>';
		$str_query.='<td width="15%">';
		$str_query.='No.';
		$str_query.='</td>';
		$str_query.='<td width="70%">';
		$str_query.='Query';
		$str_query.='</td>';
		$str_query.='<td width="15%">';
		$str_query.='Execution time';
		$str_query.='</td>';
		$str_query.='</tr>';

		foreach($_SESSION['QueryLog'] as $key=>$data)
		{
			$str_query.='<tr>';
			$str_query.='<td>';
			$str_query.=$key.')';
			$str_query.='</td>';
			$str_query.='<td>';
			$str_query.=$data['Query'];
			$str_query.='</td>';
			$str_query.='<td>';
			$str_query.=$data['ExecutionTime'].' s';
			$str_query.='</td>';
			$str_query.='</tr>';
		}
	}
	unset($_SESSION['QueryLog']);
	return $str_query;
}
?>