...
$section = 'Audio Category';
$mode = $_REQUEST['mode'];
if ($mode == "Update") {
	$audiocategoryObj->select($iAudioCategoryId);
	$audiocategoryObj->getAllVar();
} else {
	$mode = "Add";
}
...
?>
<form name="frmadd" method="post" enctype="multipart/form-data" action="index.php?file=m-audiocategoryadd_a">
    ...
    echo $GenFunctionObj->DisplayTopInListAddDealer($TOP_HEADER, 'Back to Audio Category Listing', 'index.php?file=AudioCategory&AX=Yes&iCompanyId=' . $iCompanyId . '&iSGroupId=' . $iSGroupId);
} else if ($_SESSION['sess_eType'] == "3") {
    echo $GenFunctionObj->DisplayTopInListAddDealer($TOP_HEADER, 'Back to Audio Category Listing', 'index.php?file=AudioCategory&AX=Yes&iSGroupId=' . $iSGroupId);
} else if ($_SESSION["sess_eType"] == "2") {
    echo $GenFunctionObj->DisplayTopInListAddDealer($TOP_HEADER, 'Back to Audio Category Listing', 'index.php?file=AudioCategory&AX=Yes');
}
?>
				</h1>
			</td>
		...
		if ($mode == "Update") {
            ...
            echo
            " <tr> <td valign='bottom'>" . $GeneralObj->DisplayTab('Audio', 1) . "</td>         </tr>";
        }
        ?>
		<tr>
			<td>
															...
															<table width="97%" border="0" cellpadding="3" cellspacing="5" align="center">
																...
																<tr>
																	...
																	<td width="92%" align="left">
																		<input type="text" lang="*" title="Category" name="vCategoryName" size="25" value="<?php echo $vCategoryName; ?>">
																	</td>
																</tr>
																<tr>
																...
																<tr>
																	...
																	<td style="text-align: left">
																		...
																		<?php
																		if ($_SESSION["sess_eType"] == "4") {
                                                                            echo "<input type='Image' class='buttonstyle' alt='Cancel' src='" . $CFG->wwwroot . "/public/images/btn-cancel.gif' style='cursor:hand;border:0' onclick=\"window.location='index.php?file=AudioCategory&AX=Yes&iCompanyId=" . $iCompanyId . "&iSGroupId=" . $iSGroupId . "';return false;\">";
                                                                        } else if ($_SESSION['sess_eType'] == "3") {
                                                                            echo "<input type='Image' class='buttonstyle' alt='Cancel' src='" . $CFG->wwwroot . "/public/images/btn-cancel.gif' style='cursor:hand;border:0' onclick=\"window.location='index.php?file=AudioCategory&AX=Yes&iSGroupId=" . $iSGroupId . "';return false;\">";
                                                                        } else if ($_SESSION["sess_eType"] == "2") {
                                                                            echo "<input type='Image' class='buttonstyle' alt='Cancel' src='" . $CFG->wwwroot . "/public/images/btn-cancel.gif' style='cursor:hand;border:0' onclick=\"window.location='index.php?file=AudioCategory&AX=Yes';return false;\">";
                                                                        }
                                                                        ?>
																	</td>
																</tr>
...
<?php echo " if ($mode == 'Update') {
        echo '      if($('#eStatus').val()=='Active'){
        alert('All Related Audio will be  Active if You Active this audio category.');
    }else
        if ($('#eStatus').val()=='Inactive') {
        alert('All Related Audio will be  Inactive if You Active this audio category.');
    }
 }"; ?>
     });
